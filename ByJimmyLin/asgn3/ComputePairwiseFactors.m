...
% rolling the pairwiseModel to column vector
rPairwiseModel = pairwiseModel(:);
for i = 1:n-1, % for pairwise potential, only n-1 factors
    factors(i).var = [i i+1]; 
    factors(i).card = [K K];
    factors(i).val = rPairwiseModel; % rolled representation factor as shown in samples
end
