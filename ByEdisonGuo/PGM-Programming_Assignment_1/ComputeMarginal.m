...
if(numel(E) ~= 0)
   ...
   for i = 1:length(F)
      if length(F(i).var) > 1
          ...
          indices = AssignmentToIndex(assignments(:, 2:end), F(i).card(2:end));
          for idx = unique(indices)'
               valIdx = indices == idx;
               total = sum(F(i).val(valIdx)); 
               if total ~= 0
                   F(i).val(valIdx) = F(i).val(valIdx) / sum(F(i).val(valIdx)); 
               end
