...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    betas = repmat(struct('var', [], 'card', [], 'val', []), 1, N);
    mus = repmat(struct('var', [], 'card', [], 'val', []), 1, N);
    ...
    if (doLogZ)
        ...
        for i = 1:N
            ...
            betas(i) = P.cliqueList(i);
            
            for j = neighbourIdx'
                betas(i) = FactorProduct(betas(i), unnormalizedMessages(j,i));
                mus(i) = FactorProduct(mus(i), unnormalizedMessages(j,i));
            end
        end
        
        jointBeta = struct('var', [], 'card', [], 'val', []);
        jointMu = struct('var', [], 'card', [], 'val', []);
        for i = 1:N
            jointBeta = FactorProduct(jointBeta, betas(i));
            jointMu = FactorProduct(jointMu, mus(i));
        end
      
        jointMu.val = 1 ./ jointMu.val;
        joint = FactorProduct(jointBeta, jointMu);
        
        logZ = log(sum(joint.val));
